 ...
 */
module.exports = function pictureDownloader(url, dest, timeout = 3 * 60 * 1000, retries = 2,callback) {
  let isRetry = false

  let req = null;
  if( url.indexOf('https') < 0 ){
    req = http.request(url, res => res.pipe(fs.createWriteStream(dest)))
  }else{
    req = https.request(url, res => res.pipe(fs.createWriteStream(dest)))
  }
